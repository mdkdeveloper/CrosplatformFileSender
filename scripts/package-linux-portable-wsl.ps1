...
foreach ($Argument in $args) {
    ...
    elseif (-not $VersionProvided -and -not $Argument.StartsWith("-")) {
        ...
        throw "Unknown argument: $Argument. Usage: build-linux-portable-wsl.bat [version] [--no-open]"
    }
}

$RepoRoot = Split-Path -Parent $PSScriptRoot
$OutputDir = Join-Path $RepoRoot "desktopApp\build\compose\binaries\main-release\tar"

if ($null -eq (Get-Command "wsl.exe" -ErrorAction SilentlyContinue)) {
    throw "WSL is required to build the Linux portable archive from Windows. Install WSL with a Linux distro, then run this script again."
}
...
$command = "cd $(ConvertTo-BashSingleQuoted $wslRoot) && bash scripts/package-linux-portable.sh $(ConvertTo-BashSingleQuoted $AppVersion)$openArg"

Write-Host "Building Linux portable release archive version $AppVersion through WSL..."
& wsl.exe bash -lc $command
if ($LASTEXITCODE -ne 0) {
    exit $LASTEXITCODE
}

$artifact = Get-ChildItem -Path $OutputDir -Filter "*.tar.gz" -File -ErrorAction SilentlyContinue |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1

if ($null -eq $artifact) {
    throw "Linux portable archive was not found in $OutputDir"
}

Write-Host "Linux portable package:"
